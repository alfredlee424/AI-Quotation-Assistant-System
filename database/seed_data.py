...
from database.models import Ordspd, Ordspe, Ordqty, Invdoc, Ordstr, init_db
...
#
# quo_rate 語意：報價係數（直接相乘）。目前值為 1（售價=成本×1）。
# 未來可調整為 1.3 表示 +30% 加成。
# ============================================================

INVDOC_DATA: list[dict] = [
    {
        "workgroup": WG,
        "prodkind": PFX,          # CMT1
        "codsc": "辦公桌",
        "ordkind": ORDKIND_PRODUCT,  # "1" 報價產品類別
        "quo_rate": 1.0,          # 報價係數（1=不加成，未來可調 1.3）
    },
]
...
#
# 依 logs/ss.txt 的 CMT1 樹狀結構建立 pathf→pathc 邊，
# 含 must_chose（Y=必選）與 seq（同層顯示順序）。
#
# 邊定義格式：(parent_segments, optnoc, must_chose, seq)
#   parent_segments : 父節點相對 PFX 的路徑段（() 代表根 = PFX）
#   optnoc          : 子節點選項代碼
#   must_chose      : "Y" 必選；"" 非必選
#   seq             : 同層顯示順序
# ============================================================

_ORDSTR_EDGES: list[tuple] = [
    # ── 根節點 CMT1 的直接子節點 ──────────────────────────
    ((), "A001", "Y", 1),
    ((), "B001", "",  2),
    ...
    # ── CMT1\A001\S005\S000 ───────────────────────────────
    (("A001", "S005", "S000"), "S006", "Y", 2),
    (("A001", "S005", "S000"), "S045", "Y", 3),
    (("A001", "S005", "S000"), "S052", "Y", 1),
    ...
    # ── CMT1\A001\S049\S005 ───────────────────────────────
    (("A001", "S049", "S005"), "S000", "", 3),
    (("A001", "S049", "S005"), "S001", "", 2),
    (("A001", "S049", "S005"), "S008", "", 1),

    # ── CMT1\A001\S049\S005\S000 ──────────────────────────
    (("A001", "S049", "S005", "S000"), "S051", "Y", 2),
    (("A001", "S049", "S005", "S000"), "S052", "Y", 1),

    # ── CMT1\A001\S050 ────────────────────────────────────
    (("A001", "S050"), "S005", "Y", 1),

    # ── CMT1\A001\S050\S005 ───────────────────────────────
    (("A001", "S050", "S005"), "S000", "", 3),
    (("A001", "S050", "S005"), "S001", "", 2),
    (("A001", "S050", "S005"), "S008", "", 1),

    # ── CMT1\A001\S050\S005\S000 ──────────────────────────
    (("A001", "S050", "S005", "S000"), "S051", "Y", 2),
    (("A001", "S050", "S005", "S000"), "S052", "Y", 1),
    ...
    # ── CMT1\B001\S005\S000 ───────────────────────────────
    (("B001", "S005", "S000"), "S045", "",  3),
    (("B001", "S005", "S000"), "S051", "Y", 2),
    (("B001", "S005", "S000"), "S052", "Y", 1),
...
def _build_ordstr_data() -> list[dict]:
    """由 _ORDSTR_EDGES 建立 ordstr 資料列（含 pathf/pathc/optnof/optnoc）。"""
    rows: list[dict] = []
    for parent_segments, optnoc, must_chose, seq in _ORDSTR_EDGES:
        pathf = _path(*parent_segments)          # () → PFX（根）
        pathc = _path(*parent_segments, optnoc)
        optnof = parent_segments[-1] if parent_segments else PFX
        rows.append({
            "workgroup": WG,
            "pathf": pathf,
            "pathc": pathc,
            "optnof": optnof,
            "optnoc": optnoc,
            "must_chose": must_chose or None,
            "seq": seq,
        })
    return rows


ORDSTR_DATA: list[dict] = _build_ordstr_data()
...
def seed() -> None:
    ...
    try:
        ...
        # invdoc
        for row in INVDOC_DATA:
            db.add(Invdoc(**row))

        # ordstr
        for row in ORDSTR_DATA:
            db.add(Ordstr(**row))
        ...
        print(
            ...
            f"invdoc={len(INVDOC_DATA)} 筆，"
            f"ordstr={len(ORDSTR_DATA)} 筆。"
        )
        print(
